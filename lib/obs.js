...
export function getOBSStatus() { return cached; }
...
async function refresh() {
  ...
  try {
    ...
    cached = {
      ...
      streaming: stream.outputActive, recording: record.outputActive,
      scene: scene.currentProgramSceneName,
      bitrateKbps, fps: Math.round(stats.activeFps ?? 0),
...
export async function initOBS() {
  ...
  let OBSWebSocket;
  try {
    ({ OBSWebSocket } = await import('obs-websocket-js'));
  } catch {
    console.log('[OBS]  Package not installed — run: npm install');
    return;
  }
